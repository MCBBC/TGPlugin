        ...
        if ($user != null) {
            switch (true) {
                ...
                case (substr($callback_data, 0, 4) == 'draw'):
                    ...
                    $bot->answerCallbackQuery($callback->getId(), $text = $draw->msg, $showAlert = false);
                    break;
            }
        } else {
...
未使用 ' . $user->unusedTraffic() . ' ' . number_format(($user->transfer_enable - ($user->u + $user->d)) / $user->transfer_enable * 100, 2) . '%';
    ...
    public static function telegram_process($bot, $message, $command)
    {
        ...
        if ($message->getChat()->getId() > 0) {
            ...
            switch ($command) {
                ...
                case 'help':
                    $reply['message'] = '命令列表：
						...
						/help 获取帮助信息
						/rss 获取节点订阅
						/create 流量 单位固定MB 例如/create 1024';
                    if ($user == null) {
            ...
            switch ($command) {
                ...
                case '清理红包':
                    if (!$user->is_admin) {
                        $reply['message'] = '对不起您没有权限操作。';
                        break;
                    }
                    $create = TGPlugin::Query_HB();
                    $create = json_decode($create);
                    $reply['message'] = $create->msg . ' 共失效 ' . $create->data->DataInt . ' 个红包';
                    break;
                case 'traffic':
                case 'checkin':
                case 'prpr':
                ...
                case 'rss':
                    if($message->getFrom()->getUsername()){
                        $reply['message'] = $message->getFrom()->getUsername();
                    } else {
                        $reply['message'] = $message->getFrom()->getFirstname() . ' ' . $message->getFrom()->getLastname();
                    }
                    break;
                case 'GameHelp':
                    $Game_Help = [
                        ...
                        'Game_5' => '历练',
                        'Game_6' => '突破'];
                    $reply['message'] = '========== 修仙Game ==========';
                    $keys = [];
                    foreach ($Game_Help as $key => $value) {
                        $keys[] = [['text' => $value, 'callback_data' => $key]];
                    }
                    $reply['markup'] = new InlineKeyboardMarkup(
                        $keys
    ...
    public static function process()
    {
        try {
            ...
            $command_list = array('ping', 'traffic', 'help', 'prpr', 'checkin', 'rss', '流量红包', '清理红包', 'img', 'GameHelp');
